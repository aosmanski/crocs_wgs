...
## You will need to replace cAcu with the genome assembly identifier as appropriate.
...
## cat list.txt | while read i; do sed "s/cAcu/$i/g" <new new file name> > ${i}_RM_C8.sh; done
...
python /lustre/scratch/aosmansk/new_croc_assemblies/repeatmasker/slurm_clusterrun_time48.py \
        -i ${GENOME}_ref-based_assembly.fas \
        -b 10 \
        -sp vertebrata \
        -dir . \
        -xsmall
